...
import { Trophy, AlertTriangle, Leaf, Activity, School, Award, TrendingUp, Zap, Recycle, Droplets } from "lucide-react"
...
const mockSchools: SchoolData[] = [
  { 
    ...
    score: 92, 
    rank: 1, 
    alerts: ["Excellent performance this week!"],
    certification: "platinum",
    metrics: {
      wasteManagement: 95,
      energyEfficiency: 90,
      sustainabilityProjects: 92,
      waterConservation: 88
    },
    improvements: ["Consider expanding solar panel coverage", "Implement rainwater harvesting system"]
  },
  { 
    id: 2, 
    ...
    score: 84, 
    rank: 2, 
    alerts: ["Slight increase in emissions."],
    certification: "gold",
    metrics: {
      wasteManagement: 88,
      energyEfficiency: 82,
      sustainabilityProjects: 85,
      waterConservation: 80
    },
    improvements: ["Reduce energy consumption during peak hours", "Introduce more recycling bins", "Start composting program"]
  },
  { 
    id: 3, 
    ...
    score: 76, 
    rank: 3, 
    alerts: ["Great improvement in recycling."],
    certification: "silver",
    metrics: {
      wasteManagement: 78,
      energyEfficiency: 75,
      sustainabilityProjects: 72,
      waterConservation: 78
    },
    improvements: ["Install LED lighting throughout campus", "Create student-led sustainability committee", "Reduce single-use plastics in cafeteria"]
  },
  { 
    id: 4, 
    ...
    score: 61, 
    rank: 4, 
    alerts: ["Energy usage is too high."],
    certification: "bronze",
    metrics: {
      wasteManagement: 65,
      energyEfficiency: 58,
      sustainabilityProjects: 60,
      waterConservation: 62
    },
    improvements: ["Upgrade HVAC systems for efficiency", "Implement stricter waste segregation", "Launch tree planting initiative", "Reduce paper usage"]
  },
  { 
    id: 5, 
    ...
    score: 52, 
    rank: 5, 
    alerts: ["Immediate action required on waste reduction."],
    certification: "none",
    metrics: {
      wasteManagement: 48,
      energyEfficiency: 52,
      sustainabilityProjects: 55,
      waterConservation: 50
    },
    improvements: ["Establish baseline sustainability goals", "Train staff on energy conservation", "Install motion-sensor lighting", "Create waste audit system", "Partner with local recycling facilities"]
  },
]
...
const certificationConfig: Record<CertificationType, {
  label: string
  color: string
  icon: string
  description: string
  ...
  silver: { 
    label: "Silver Green School", 
    color: "bg-gradient-to-r from-gray-400/20 to-gray-500/20 text-gray-300 border-gray-400/40",
    icon: "🥈",
    description: "Strong commitment to sustainability"
  },
  bronze: { 
    label: "Bronze Green School", 
    color: "bg-gradient-to-r from-orange-600/20 to-orange-700/20 text-orange-300 border-orange-600/40",
    icon: "🥉",
    description: "Good sustainability practices"
  },
  none: { 
    label: "Not Certified", 
    color: "bg-muted/20 text-muted-foreground border-muted/40",
    icon: "📋",
    description: "Working towards certification"
  }
}
          ...
          <div className="space-y-4">
            {schools.map((school) => (
              <div
                ...
                className="glass rounded-lg p-4 glass-hover cursor-pointer transition-all duration-200"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <Badge variant="outline" className={`${certificationConfig[school.certification].color} text-xs`}>
                      {certificationConfig[school.certification].icon} {certificationConfig[school.certification].label}
                    </Badge>
                  </div>
